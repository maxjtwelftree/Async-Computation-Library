...
def stat_arb_success(stock_test, net):
    ...
    for i in range(int(len(stock_test[0])/10)):
        ...
        upper_bound = net(torch.FloatTensor(query_data))[0]
...
class Net_independent(nn.Module):
    ...
    def forward(self, x):
        ...
        return [torch.cat((self.delta0_s[i]*torch.ones((x.shape[0],1)),delta_list[i]), axis=1) for i in range(z)], self.c
